/**
* @file pc_communication.c
* @brief Code for communication over the APBUART
* @details Functions related to communication over the APBUART is located in this file.
* Several help functions are here to read the message, store it, validate
* and handle errors in the communication.
* @author David Alm
* @version 1.0
*/
...
//Macro
#define MAX_MSG_SIZE 129	///< Current maximum allowed message size, 128 ASCII + terminator.
#define MSG_TERMINATOR 'x'	///< Messages always end with the terminator, select it with this macro.
...
char uart_buffer[MAX_MSG_SIZE] = { 0 };	///< Character buffer where messages are stored.
int msg_length = 13;					///< Stores the length of the current message, initial and reset value is 13.
int uart_buffer_index = 0;				///< Index keeper for the message buffer.
int send_index = 0;						///< When sending messages this index to track next character to send.
bool not_sending = true;				///< Bool used to track if we are transmitting a message.
bool receiving = false;					///< Bool used to track if we are receiving a messge.


/**
* Initializes the PC communication and activates the UART. It maps the
* PcConnectionHandler function to be called when interrupts are generated by the UART.
*/
...
/**
* This function is set to be called when interrupts are generated by the UART
* when PcConnectionInit is called. It will determine the if a character has been
* received, if the UART is ready to send a new character and checks if errors
* in the transmission has occurred.
*/
...
/**
* Checks the type of message and will perform the appropriate action. It will
* format the content of the message to appropriate format and call configuration functions
* for filters, tone-control and echo effect.
*/
static void MessageHandler() {
...
/**
* Will check the type of record currently stored in the UART buffer.
* @return Corresponding Enum for the type of record received.
*/
static input_type_e GetMessageTypeEnum() {
...
/**
* This function is used when sending character. It sends one character per interrupt
* and when complete will ask for reset.
* @return True if reset should be performed otherwise false.
*/
static bool SendAChar() {
...
/**
* Reads from the UART when called and stores the message in the UART buffer.
* it will check that the message is correctly received and if not will
* signal that an error has occurred and requests a reset.
* @return True if reset should be performed otherwise false.
*/
static bool ReadFromUart() {
...
/**
* When a message has been sent or received and fully handled this function is
* called to reset critical variables to initial values.
*/
static void ResetPcConnectionParameters() {
...
/**
* Help function to calculate the checksum and compare it to the
* received checksum in the message.
* @return True if checksum is valid and otherwise false.
*/
static bool VerifyChecksum() {
